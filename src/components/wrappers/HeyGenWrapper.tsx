...
export default function HeyGenWrapper({ children }: { children: React.ReactNode }) {
    ...
    const _showModal = useMemo(() => {
        return (heyGenAPIKey != null && !heyGenAPIKey) || invalid_heygen_api_key != null;
    }, [heyGenAPIKey, invalid_heygen_api_key]);

    return <>
        {_showModal ? <>
            <Modal size="lg" isOpen={_showModal} hideCloseButton scrollBehavior="inside">
                <ModalContent>
                    <ModalHeader className="flex flex-col gap-1">
                        ...
                        {
                            invalid_heygen_api_key == 'unauth-401' &&
                            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mt-2 rounded relative" role="alert">
                                <strong className="font-bold">Invalid HeyGen API Key (Unauthorized) </strong>
                            </div>
                        }
                    </ModalHeader>
                    <ModalBody>
